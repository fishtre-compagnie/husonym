		...
		if !d.IsDir() && filepath.Ext(path) == ".go" {
			...
			for _, cgroup := range node.Comments {
				for _, comment := range cgroup.List {
					if strings.HasPrefix(comment.Text, "// +husonymTransformerBuilder:") {
...
const newValue = husonym.{{.BenthosSpec.Name}}(value, {});
{{- else }}
const newValue = husonym.{{.BenthosSpec.Name}}(value, {
...
{{if eq (len .BenthosSpec.Params) 0}}
const newValue = husonym.{{.BenthosSpec.Name}}({});
{{- else }}
const newValue = husonym.{{.BenthosSpec.Name}}({
	{{- range $i, $param := .BenthosSpec.Params -}}
	{{ if $param.HasDefault }}
...
func generateExample(bs *transformers.BenthosSpec) (string, error) {
	...
	t := template.Must(template.New("husonymTransformerExample").Parse(exampleTemplate))
...
hide_title: false
id: javascript
description: Learn about Husonym's javascript transformer
---
<!-- prettier-ignore-start -->
<!--
	Code generated by Husonym husonym_js_transformer_docs_generator.go. DO NOT EDIT.
-->

# Husonym Javascript Transformer Functions

Learn about Husonym's Javascript transformer and generator functions, which provide a wide range of capabilities for data transformation and
...
Husonym's transformer functions allow you to manipulate and transform data values with ease.
...
Husonym's generator functions enable the creation of various data values, facilitating the generation of realistic and diverse data for
