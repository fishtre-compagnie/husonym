...
var (
	reCreateTable = regexp.MustCompile(
		`CREATE\s+TABLE\s+IF\s+NOT\s+EXISTS\s+(\w+)\s*\.\s*(\w+)\s*\(`,
	)
	...
	for _, line := range lines {
		...
		if matches := reCreateTable.FindStringSubmatch(line); len(matches) > 2 {
			...
			if matches := reColumn.FindStringSubmatch(line); len(matches) > 1 {
				...
				if slices.Contains(
					[]string{"primary key", "constraint", "key", "unique", "primary", "alter"},
...
func formatJobMappings(
	pkgName, sqlFile string,
	tables []*Table,
	generateTypeMap bool,
) (string, error) {
	const tmpl = `
// Code generated by Husonym jobmapping_generator. DO NOT EDIT.
// source: {{ .SourceFile }}
...
func main() {
	...
	for _, input := range inputs {
		...
		formattedJobMappings, err := formatJobMappings(
			goPkgName,
			input.SqlFile,
			tables,
