...
func generateCode(pkgName string, funcInfo *transformers.BenthosSpec) (string, error) {
	...
	t := template.Must(template.New("husonymTransformerImpl").Parse(codeTemplate))
...
func generateBenchmarkCode(
	...
	t := template.Must(template.New("husonymTransformerBenchmarkImpl").Parse(benchmarkTemplate))
	var out bytes.Buffer
	err := t.Execute(&out, data)
	if err != nil {
		return "", err
