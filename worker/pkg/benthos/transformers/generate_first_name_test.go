...
func Test_GenerateRandomFirstName(t *testing.T) {
	...
	assert.LessOrEqual(
		...
		int64(len(res)),
		maxCharacterLimit,
		"The first name should be less than or equal to the max character limit",
	)
}
...
func Test_GenerateRandomFirstName_Clamped(t *testing.T) {
	randomizer := rng.New(1)
	res, err := generateRandomFirstName(randomizer, gotypeutil.ToPtr(int64(10)), maxCharacterLimit)
