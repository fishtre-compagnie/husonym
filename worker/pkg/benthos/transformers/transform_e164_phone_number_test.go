...
func Test_TransformE164NumberPreserveLengthTrue(t *testing.T) {
	...
	assert.Equal(
		t,
		validateE164(*res),
		validateE164(testE164Phone),
		"The expected value should be a valid e164 number.",
	)
	assert.Equal(
		t,
		len(*res),
...
func Test_GenerateE164FormatPhoneNumberPreserveLength(t *testing.T) {
	res, err := generateE164FormatPhoneNumberPreserveLength(
...
func Test_TransformE164NumberWithoutPlusSign(t *testing.T) {
	...
	t.Run("preserve length true", func(t *testing.T) {
		res, err := transformE164PhoneNumber(
			rng.New(time.Now().UnixNano()),
			phoneWithoutPlus,
			true,
			nil,
