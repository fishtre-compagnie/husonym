...
func Test_GenerateRandomLastName(t *testing.T) {
	...
	assert.LessOrEqual(
		...
		int64(len(res)),
		maxCharacterLimit,
		"The last name should be less than or equal to the max character limit",
	)
}
...
func Test_GenerateRandomLastName_Clamped(t *testing.T) {
	randomizer := rng.New(1)
	res, err := generateRandomLastName(randomizer, gotypeutil.ToPtr(int64(10)), maxCharacterLimit)
...
func Test_GenerateRandomLastNameTransformer(t *testing.T) {
	...
	assert.LessOrEqual(
		...
		int64(len(resStr)),
		maxCharacterLimit,
		"The last name should be less than or equal to max char limit",
	)
}
