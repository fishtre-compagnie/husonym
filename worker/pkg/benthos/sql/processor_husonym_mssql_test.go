...
func Test_transformHusonymToMssql(t *testing.T) {
	...
	t.Run("handles basic values", func(t *testing.T) {
		...
		result, err := transformHusonymToMssql(input, columns, columnDefaultProperties)
	...
	t.Run("returns error for invalid root type", func(t *testing.T) {
		result, err := transformHusonymToMssql("invalid", columns, columnDefaultProperties)
		require.Error(t, err)
...
func Test_HusonymToMssqlProcessor(t *testing.T) {
	conf := `
columns:
  - id
  - name
...
`
	spec := husonymToMssqlProcessorConfig()
	env := service.NewEnvironment()

	procConfig, err := spec.ParseYAML(conf, env)
	require.NoError(t, err)

	proc, err := newHusonymToMssqlProcessor(procConfig, service.MockResources())
	require.NoError(t, err)
...
func Test_HusonymToMssqlProcessor_SubsetColumns(t *testing.T) {
	conf := `
columns:
  - id
  - name
