...
func husonymToPgxProcessorConfig() *service.ConfigSpec {
...
func RegisterHusonymToPgxProcessor(env *service.Environment) error {
	return env.RegisterBatchProcessor(
		"husonym_to_pgx",
		husonymToPgxProcessorConfig(),
		func(conf *service.ParsedConfig, mgr *service.Resources) (service.BatchProcessor, error) {
			proc, err := newHusonymToPgxProcessor(conf, mgr)
			if err != nil {
				return nil, err
			}
			return proc, nil
...
type husonymToPgxProcessor struct {
...
func newHusonymToPgxProcessor(
	conf *service.ParsedConfig,
	mgr *service.Resources,
) (*husonymToPgxProcessor, error) {
...
func (p *husonymToPgxProcessor) ProcessBatch(
	ctx context.Context,
	...
	for _, msg := range batch {
		...
		newRoot, err := transformHusonymToPgx(
			root,
			p.columns,
			p.columnDataTypes,
...
func (m *husonymToPgxProcessor) Close(context.Context) error {
	return nil
}
func transformHusonymToPgx(
	root any,
	columns []string,
	columnDataTypes map[string]string,
	columnDefaultProperties map[string]*husonym_benthos.ColumnDefaultProperties,
...
func getPgxValue(
	...
	value, isHusonymValue, err := getPgxHusonymValue(value)
	if err != nil {
		return nil, err
	}
	if isHusonymValue {
...
func getPgxHusonymValue(root any) (value any, isHusonymValue bool, err error) {
	if valuer, ok := root.(husonymtypes.HusonymPgxValuer); ok {
		value, err := valuer.ValuePgx()
		if err != nil {
			return nil, false, fmt.Errorf("unable to get PGX value from HusonymPgxValuer: %w", err)
		}
...
func getColumnDefaultProperties(
	...
	for key, properties := range columnDefaultPropertiesConfig {
		...
		var colDefaults husonym_benthos.ColumnDefaultProperties
