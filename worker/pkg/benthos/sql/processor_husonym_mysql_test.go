...
func Test_transformHusonymToMysql(t *testing.T) {
	...
	t.Run("handles basic values", func(t *testing.T) {
		...
		result, err := transformHusonymToMysql(
			input,
			columns,
			columnDataTypes,
...
func Test_getMysqlValue(t *testing.T) {
	t.Run("returns default for column with default transformer", func(t *testing.T) {
		colDefaults := &husonym_benthos.ColumnDefaultProperties{HasDefaultTransformer: true}
...
func Test_HusonymToMysqlProcessor(t *testing.T) {
	conf := `
columns:
  - id
  - name
...
`
	spec := husonymToMysqlProcessorConfig()
	env := service.NewEnvironment()

	procConfig, err := spec.ParseYAML(conf, env)
	require.NoError(t, err)

	proc, err := newHusonymToMysqlProcessor(procConfig, service.MockResources())
	require.NoError(t, err)
...
func Test_HusonymToMysqlProcessor_SubsetColumns(t *testing.T) {
	conf := `
columns:
  - id
  - name
