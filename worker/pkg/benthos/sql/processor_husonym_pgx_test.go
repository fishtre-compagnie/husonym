...
func Test_transformHusonymToPgx(t *testing.T) {
	...
	t.Run("transforms values correctly", func(t *testing.T) {
		...
		got, err := transformHusonymToPgx(input, columns, columnDataTypes, columnDefaultProperties)
		require.NoError(t, err)
...
func Test_HusonymToPgxProcessor(t *testing.T) {
	conf := `
columns:
  - id
  - name
...
`
	spec := husonymToPgxProcessorConfig()
	env := service.NewEnvironment()

	procConfig, err := spec.ParseYAML(conf, env)
	require.NoError(t, err)

	proc, err := newHusonymToPgxProcessor(procConfig, service.MockResources())
	require.NoError(t, err)

	interval, err := husonymtypes.NewInterval()
	require.NoError(t, err)
...
func Test_HusonymToPgxProcessor_SubsetColumns(t *testing.T) {
	conf := `
columns:
  - id
  - name
...
`
	...
	msgMap := map[string]any{
		...
		"tags":       []string{"tag1", "tag2"},
		"metadata":   map[string]string{"key": "value"},
		"interval": husonymtypes.Interval{
			Months:       1,
			Days:         10,
			Microseconds: 3600000000,
		},
