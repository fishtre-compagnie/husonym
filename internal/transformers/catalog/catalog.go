// Package catalog holds the system transformers and the config each one starts with. The
// backend serves it; the worker reads it to map a new column the way the UI would.
package catalog
...
// enterprise ones too when licensed. The slice is shared: callers must not modify it.
func Transformers(licensed bool) []*mgmtv1alpha1.SystemTransformer {
	if licensed {
...
// BySource indexes Transformers by source. The map is shared: callers must not modify it.
func BySource(licensed bool) map[mgmtv1alpha1.TransformerSource]*mgmtv1alpha1.SystemTransformer {
	if licensed {
...
// DefaultConfig returns a copy of the config a system transformer starts with — what the UI
// applies when someone picks it — or false when the source is not in the catalogue.
func DefaultConfig(source mgmtv1alpha1.TransformerSource, licensed bool) (*mgmtv1alpha1.TransformerConfig, bool) {
	transformer, ok := BySource(licensed)[source]
	if !ok {
		return nil, false
	}
	return proto.CloneOf(transformer.GetConfig()), true
}
