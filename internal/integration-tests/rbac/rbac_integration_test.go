...
func TestRbac(t *testing.T) {
	...
	t.Run("account_admin", func(t *testing.T) {
		...
		err = rbacclient.SetAccountRole(
			ctx,
			rbac.NewUserIdEntity(userId),
		...
		err = rbacclient.EnforceJob(
			ctx,
			rbac.NewUserIdEntity(userId),
			rbac.NewAccountIdEntity(accountId),
			rbac.NewJobIdEntity(jobId),
		...
		err = rbacclient.EnforceConnection(
			ctx,
			rbac.NewUserIdEntity(userId),
			rbac.NewAccountIdEntity(accountId),
			rbac.NewConnectionIdEntity(connectionId),
	...
	t.Run("job_developer", func(t *testing.T) {
		...
		err = rbacclient.SetAccountRole(
			...
			rbac.NewUserIdEntity(userId),
			rbac.NewAccountIdEntity(accountId),
			mgmtv1alpha1.AccountRole_ACCOUNT_ROLE_JOB_DEVELOPER,
		)
		require.NoError(t, err)
	...
	t.Run("job_executor", func(t *testing.T) {
		...
		err = rbacclient.SetAccountRole(
			...
			rbac.NewUserIdEntity(userId),
			rbac.NewAccountIdEntity(accountId),
			mgmtv1alpha1.AccountRole_ACCOUNT_ROLE_JOB_EXECUTOR,
		)
		require.NoError(t, err)
	...
	t.Run("job_viewer", func(t *testing.T) {
		...
		err = rbacclient.SetAccountRole(
			...
			rbac.NewUserIdEntity(userId),
			rbac.NewAccountIdEntity(accountId),
			mgmtv1alpha1.AccountRole_ACCOUNT_ROLE_JOB_VIEWER,
		)
		require.NoError(t, err)
	...
	t.Run("role_changes", func(t *testing.T) {
		...
		err = rbacclient.RemoveAccountRole(
			ctx,
			rbac.NewUserIdEntity(userId),
			rbac.NewAccountIdEntity(accountId),
			mgmtv1alpha1.AccountRole_ACCOUNT_ROLE_ADMIN,
	...
	t.Run("cross_account_access", func(t *testing.T) {
		...
		err = rbacclient.SetAccountRole(
			ctx,
			rbac.NewUserIdEntity(user1Id),
			rbac.NewAccountIdEntity(account1Id),
			mgmtv1alpha1.AccountRole_ACCOUNT_ROLE_ADMIN,
		)
		require.NoError(t, err)
		...
		err = rbacclient.SetAccountRole(
			ctx,
			rbac.NewUserIdEntity(user2Id),
			rbac.NewAccountIdEntity(account2Id),
			mgmtv1alpha1.AccountRole_ACCOUNT_ROLE_ADMIN,
		)
		require.NoError(t, err)
	...
	t.Run("mixed_roles_same_account", func(t *testing.T) {
		...
		err = rbacclient.SetAccountRole(
			ctx,
			rbac.NewUserIdEntity(adminUserId),
			rbac.NewAccountIdEntity(accountId),
			mgmtv1alpha1.AccountRole_ACCOUNT_ROLE_ADMIN,
		)
		require.NoError(t, err)
		...
		err = rbacclient.SetAccountRole(
			ctx,
			rbac.NewUserIdEntity(viewerUserId),
			rbac.NewAccountIdEntity(accountId),
			mgmtv1alpha1.AccountRole_ACCOUNT_ROLE_JOB_VIEWER,
		)
		require.NoError(t, err)
