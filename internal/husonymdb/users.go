...
type upsertPersonalAccountResponse struct {
	Account *db_queries.HusonymApiAccount
}
...
func (d *HusonymDb) CreateTeamAccount(
	ctx context.Context,
	userId pgtype.UUID,
	teamName string,
	logger *slog.Logger,
) (*db_queries.HusonymApiAccount, error) {
	var teamAccount *db_queries.HusonymApiAccount
	if err := d.WithTx(ctx, &pgx.TxOptions{IsoLevel: pgx.Serializable}, func(dbtx BaseDBTX) error {
		accounts, err := d.Q.GetAccountsByUser(ctx, dbtx, userId)
		if err != nil && !IsNoRows(err) {
			return fmt.Errorf("unable to get account(s) by user id: %w", err)
		} else if err != nil && IsNoRows(err) {
			accounts = []db_queries.HusonymApiAccount{}
		}
...
type ConvertPersonalToTeamAccountResponse struct {
	PersonalAccount *db_queries.HusonymApiAccount
	TeamAccount     *db_queries.HusonymApiAccount
}

func (d *HusonymDb) ConvertPersonalToTeamAccount(
	ctx context.Context,
...
func (d *HusonymDb) UpsertStripeCustomerId(
	ctx context.Context,
	accountId pgtype.UUID,
	getStripeCustomerId func(ctx context.Context, account db_queries.HusonymApiAccount) (string, error),
	logger *slog.Logger,
) (*db_queries.HusonymApiAccount, error) {
	var account *db_queries.HusonymApiAccount
...
func (d *HusonymDb) CreateTeamAccountInvite(
	ctx context.Context,
	accountId pgtype.UUID,
	userId pgtype.UUID,
	email string,
