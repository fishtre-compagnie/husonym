...
func Test_HusonymArray(t *testing.T) {
	t.Run("NewHusonymArray initializes with correct defaults", func(t *testing.T) {
		elements := []HusonymAdapter{
			&Interval{},
			&Interval{},
		}
		array, err := NewHusonymArray(elements)
		require.NoError(t, err)
		require.NotNil(t, array)
		require.Equal(t, HusonymArrayId, array.Husonym.TypeId)
		require.Equal(t, LatestVersion, array.GetVersion())
		require.Equal(t, len(elements), len(array.Elements))
	})

	t.Run("NewHusonymArray accepts options", func(t *testing.T) {
		elements := []HusonymAdapter{&Interval{}}
		customVersion := Version(1)
		array, err := NewHusonymArray(elements, WithVersion(customVersion))
		require.NoError(t, err)
		require.Equal(t, customVersion, array.GetVersion())
	})

	t.Run("NewHusonymArray with invalid version returns error", func(t *testing.T) {
		elements := []HusonymAdapter{&Interval{}}
		invalidVersion := Version(999)
		array, err := NewHusonymArray(elements, WithVersion(invalidVersion))
		require.Error(t, err)
		require.Nil(t, array)
		require.Contains(t, err.Error(), "invalid Husonym Type version")
	})

	t.Run("ScanPgx handles slice type mismatch", func(t *testing.T) {
		array, err := NewHusonymArray([]HusonymAdapter{&Interval{}})
		require.NoError(t, err)
	...
	t.Run("ScanPgx handles valid intervals", func(t *testing.T) {
		interval1 := &Interval{}
		interval2 := &Interval{}
		array, err := NewHusonymArray([]HusonymAdapter{interval1, interval2})
		require.NoError(t, err)
