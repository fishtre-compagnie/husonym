---
title: Developing Husonym Locally
description: Learn how to develop with Husonym Open Source locally in order to get up to speed with how Husonym works
id: husonym-local-dev
hide_title: false
slug: /guides/husonym-local-dev
---

## Introduction

This section goes into detail on each tool that is used for developing with Husonym locally.

## Husonym DevContainer

Husonym has a pre-published [devcontainer](https://containers.dev/) that can be used to easily get a working Husonym dev environment.
This container comes pre-packaged with all of the tools needed for developing Husonym, and works with Tilt or Compose, or Bare Metal setups.
...
Husonym uses Buf to generate code from our proto files. This is possible to do unauthenticated, but if done often (more than 10 requests in an hour), you will be rate limited. To combat this, you must login to the [BSR](https://buf.build) and create a user token.
...
The allow list can be found by first opening Docker Desktop. `Settings -> Resources -> File Sharing` and add the path to the Husonym repository.
...
The cluster is declaratively defined [here](https://github.com/fishtre-compagnie/husonym/tree/main//tilt/kind/cluster.yaml)

The below command invokes the cluster-create script that can be found [here](https://github.com/fishtre-compagnie/husonym/tree/main//tilt/scripts/cluster-create.sh)

```
make cluster/create
```

After the cluster has been successfully created, `tilt up` can be run to start up `husonym`.
Refer to the top-level [Tiltfile](https://github.com/fishtre-compagnie/husonym/tree/main//Tiltfile) for a clear picture of everything that runs.
Each dependency in the `husonym` repo is split into sub Tilt files so that they can be run in isolation, or in combination with other sub-resources more easily.

Once everything is up and running, the app can be accessed locally at [http://localhost:3000](http://localhost:3000).

## Developing on Bare Metal

You can develop Husonym totally on bare metal. Every service supports a .env file along with environment specific .env overrides.
...
This section contains a flat list of the tools that are used to develop Husonym and why.
...
Tilt is a great tool that is used to automate the setup of a Kubernetes cluster. There are multiple `Tiltfile`'s throughout the code, along with a top-level one that is used to inject all of the K8s manifests to setup Husonym inside of a K8s cluster.

This enables fast development, locally, while closely mimicking a real production environment.

- [kind](https://github.com/kubernetes-sigs/kind)
  - Kubernetes in Docker. We use this to spin up a slim kubernetes cluster that deploys all of the `husonym` resources.
