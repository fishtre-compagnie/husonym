...
MongoDB is one of the most commonly used NoSQL database in the world and Husonym natively supports it.
...
Husonym's compose file has an option to stand up a local MongoDB instance. If you're using the `compose.dev.yml` file to stand up Husonym locally, uncomment the MongoDb line in order to stand up a MongoDB instance:
...
- **URL**: Enter your database connection url that will be used to connect to Mongo. Husonym supports both `mongodb` and `mongodb+srv` protocols.
...
Your setup will vary based on your specific settings, but generally, you can configure Husonym to connect to Mongo by providing a Client Certificate and Client Key in their respective fields in the Connection(s) form.

By doing so, Husonym will store them on disk and update the connection url to point to those files so that they can be used to connect.
...
Once you've configured your Connection, you can test to make sure that it works by pressing the **Test Connection** button. If Husonym is able to connect to it, then you should see something like:
...
When configuring your MongoDB user, please ensure that you grant it read permissions to relevant Database Collections that you want Husonym to have access to.

### Destination Connections

When configuring your MongoDB user, please ensure that you grant it necessary write permission to the relevant Database and Collections that Husonym will need access to.
