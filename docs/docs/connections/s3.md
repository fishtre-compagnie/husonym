---
title: S3
description: Learn how to use Husonym to sync data from a database to AWS S3
id: s3
hide_title: false
slug: /connections/s3
---
...
![s3](https://assets.fishtre-compagnie.com/husonym/docs/s3.png)
...
When data is synced to your S3 bucket using Husonym, the output will be stored in JSON arrays compressed into gz files. The files are organized in the following path structure:
...
This format allows Husonym CLI to easily stream records from the file line by line as opposed to downloading the entire json blob, decoding it, and then beginning record insertion.
