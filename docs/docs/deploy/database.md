---
title: Husonym Database Setup
description: Learn how to configure a Postgres database for use with Husonym
id: database
hide_title: false
...
This section provides some color to the Postgres environment that is required to run the Husonym database migrations.
...
Husonym API comes equipped with a set of SQL schema migration files that are used to assert the Husonym database on boot up.
...
As a part of this, one may run Husonym API in the `DB_AUTO_MIGRATE=true` mode, where, on startup, Husonym API will run the database migration scripts prior to binding to its port. The same credentials used to do this will be what are used to generally connect to the Husonym API during standard crud operations.
...
Husonym uses [golang-migrate](https://github.com/golang-migrate/migrate) to handle running database migrations. We wrap it inside of our own CLI so that we can easily run it in containerized environments.
...
This table can be changed by providing the `DB_MIGRATIONS_TABLE` environment variable. For example: `DB_MIGRATIONS_TABLE=husonym_api_schema_migrations`.
...
```sql
CREATE ROLE husonym_owner WITH
        NOSUPERUSER
        NOCREATEDB
        NOCREATEROLE
        INHERIT
...
```sql
GRANT CREATE ON SCHEMA public TO husonym_owner;
GRANT USAGE ON SCHEMA public TO husonym_owner;
GRANT CREATE ON DATABASE postgres TO husonym_owner;
```

We can run the migrations with an admin user that inherits from the `husonym_owner` role.

```sql
CREATE ROLE husonym_admin WITH
    LOGIN
    INHERIT
    PASSWORD 'password'
    NOSUPERUSER
    ...
    CONNECTION LIMIT -1
    VALID UNTIL 'infinity';
GRANT husonym_owner TO husonym_user;
```

In order to retain the ownership of the tables to the `husonym_owner`, you'll need to ensure the `DB_MIGRATIONS_OPTIONS=-c role=husonym_owner` is set.
