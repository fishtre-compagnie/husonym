...
In this quick start, we're going to walk through how you can seed a PostgreSQL database with synthetic data for better data security and privacy in your staging and local environments. This guide is meant to be the shortest path to setting up Husonym and running your first Job.

For this quick start, we're going to use [Neon](https://neon.tech) as our PostgreSQL provider. Husonym supports any PostgreSQL compatible database. Husonym also supports MySQL compatible databases. However, this quick start will focus on Postgres but the two can be configured very similarly.
...
Nice! Okay, last step for Neon. Let's get our connection string so we can connect to Neon from Husonym.
...
![husonym-connect-form](/img/neon-integration.png)
...
![husonym-test](/img/neon-test.png)

Click on **Submit** to move onto the last part.

### Creating a Job

In order to generate data, we need to create a **Job** in Husonym. Let's click on **Jobs** and then click on **+ New Job**. We're now presented with two options:

![husonym-test](/img/data-gen.png)
...
![husonym-test](/img/neon-connect.png)
...
![husonym-test](/img/rows-table.gif)

Once you've completed this, you'll see the **Transformer Mapping** table populate with the columns of the table.

Lastly, we need to determine what kind of synthetic data we want to create and map that to our schema. Husonym has **Transformers** which are ways of creating synthetic data. Click on the **Select a Transformer** button in the **Transformer Mapping** table row to select a Transformer.
...
![husonym-test](/img/update-age.gif)
...
![husonym-test](/img/success-job.png)
...
![husonym-test](/img/users-count.png)
...
![husonym-test](/img/data-users.png)
...
Lastly, if you want to anonymize existing data then it's a similar workflow. The only difference is that you select the [Data Sync Job](/guides/creating-a-sync-job) and select a destination database.
