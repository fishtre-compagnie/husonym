CREATE TABLE IF NOT EXISTS husonym_api.slack_oauth_connections (
...
CREATE INDEX IF NOT EXISTS idx_slack_oauth_connections_account_id
  ON husonym_api.slack_oauth_connections(account_id);

CREATE TRIGGER update_husonym_api_slack_oauth_connections_updated_at
  BEFORE UPDATE ON husonym_api.slack_oauth_connections
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

COMMENT ON TABLE husonym_api.slack_oauth_connections
