CREATE TABLE IF NOT EXISTS husonym_api.account_hooks (
...
CREATE INDEX idx_account_hooks_account_enabled
  ON husonym_api.account_hooks(account_id, enabled)
  WHERE enabled = true;

-- Create a GIN index for events with the partial condition
CREATE INDEX idx_account_hooks_events_lookup
  ON husonym_api.account_hooks USING GIN (events)
  WHERE enabled = true;

CREATE TRIGGER update_husonym_api_accounthooks_updated_at
  BEFORE UPDATE ON husonym_api.account_hooks
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

COMMENT ON TABLE husonym_api.account_hooks
