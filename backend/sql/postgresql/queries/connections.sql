...
SELECT * from husonym_api.connections WHERE id = $1;

-- name: GetConnectionsByIds :many
SELECT * from husonym_api.connections WHERE id = ANY($1::uuid[]);

-- name: GetConnectionByNameAndAccount :one
SELECT c.* from husonym_api.connections c
INNER JOIN husonym_api.accounts a ON a.id = c.account_id
...
DELETE FROM husonym_api.connections WHERE id = $1;

-- name: RemoveConnectionByNameAndAccount :exec
DELETE FROM husonym_api.connections WHERE name = $1 and account_id = $2;

-- name: CreateConnection :one
INSERT INTO husonym_api.connections (
...
UPDATE husonym_api.connections
...
SELECT count(c.id) from husonym_api.connections c
INNER JOIN husonym_api.accounts a ON a.id = c.account_id
WHERE a.id = sqlc.arg('accountId') and c.name = sqlc.arg('connectionName');
