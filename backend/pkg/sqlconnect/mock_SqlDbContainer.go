// Code generated by mockery; DO NOT EDIT.
// github.com/vektra/mockery
// template: testify

package sqlconnect

import (
	"github.com/fishtre-compagnie/husonym/backend/pkg/sqldbtx"
	mock "github.com/stretchr/testify/mock"
)

// NewMockSqlDbContainer creates a new instance of MockSqlDbContainer. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewMockSqlDbContainer(t interface {
	mock.TestingT
	Cleanup(func())
}) *MockSqlDbContainer {
	if helper, ok := t.(interface{ Helper() }); ok {
		helper.Helper()
	}

	mock := &MockSqlDbContainer{}
	mock.Mock.Test(t)

	t.Cleanup(func() {
		if helper, ok := t.(interface{ Helper() }); ok {
			helper.Helper()
		}
		mock.AssertExpectations(t)
	})

	return mock
}

// MockSqlDbContainer is an autogenerated mock type for the SqlDbContainer type
type MockSqlDbContainer struct {
	mock.Mock
}

type MockSqlDbContainer_Expecter struct {
	mock *mock.Mock
}

func (_m *MockSqlDbContainer) EXPECT() *MockSqlDbContainer_Expecter {
	return &MockSqlDbContainer_Expecter{mock: &_m.Mock}
}

// Close provides a mock function for the type MockSqlDbContainer
func (_mock *MockSqlDbContainer) Close() error {
	ret := _mock.Called()

	if len(ret) == 0 {
		panic("no return value specified for Close")
	}

	var r0 error
	if returnFunc, ok := ret.Get(0).(func() error); ok {
		r0 = returnFunc()
	} else {
		r0 = ret.Error(0)
	}
	return r0
}

// MockSqlDbContainer_Close_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Close'
type MockSqlDbContainer_Close_Call struct {
	*mock.Call
}

// Close is a helper method to define mock.On call
func (_e *MockSqlDbContainer_Expecter) Close() *MockSqlDbContainer_Close_Call {
	return &MockSqlDbContainer_Close_Call{Call: _e.mock.On("Close")}
}

func (_c *MockSqlDbContainer_Close_Call) Run(run func()) *MockSqlDbContainer_Close_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *MockSqlDbContainer_Close_Call) Return(err error) *MockSqlDbContainer_Close_Call {
	_c.Call.Return(err)
	return _c
}

func (_c *MockSqlDbContainer_Close_Call) RunAndReturn(run func() error) *MockSqlDbContainer_Close_Call {
	_c.Call.Return(run)
	return _c
}

// Open provides a mock function for the type MockSqlDbContainer
func (_mock *MockSqlDbContainer) Open() (sqldbtx.DBTX, error) {
	ret := _mock.Called()

	if len(ret) == 0 {
		panic("no return value specified for Open")
	}

	var r0 sqldbtx.DBTX
	var r1 error
	if returnFunc, ok := ret.Get(0).(func() (sqldbtx.DBTX, error)); ok {
		return returnFunc()
	}
	if returnFunc, ok := ret.Get(0).(func() sqldbtx.DBTX); ok {
		r0 = returnFunc()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(sqldbtx.DBTX)
		}
	}
	if returnFunc, ok := ret.Get(1).(func() error); ok {
		r1 = returnFunc()
	} else {
		r1 = ret.Error(1)
	}
	return r0, r1
}

// MockSqlDbContainer_Open_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Open'
type MockSqlDbContainer_Open_Call struct {
	*mock.Call
}

// Open is a helper method to define mock.On call
func (_e *MockSqlDbContainer_Expecter) Open() *MockSqlDbContainer_Open_Call {
	return &MockSqlDbContainer_Open_Call{Call: _e.mock.On("Open")}
}

func (_c *MockSqlDbContainer_Open_Call) Run(run func()) *MockSqlDbContainer_Open_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *MockSqlDbContainer_Open_Call) Return(dBTX sqldbtx.DBTX, err error) *MockSqlDbContainer_Open_Call {
	_c.Call.Return(dBTX, err)
	return _c
}

func (_c *MockSqlDbContainer_Open_Call) RunAndReturn(run func() (sqldbtx.DBTX, error)) *MockSqlDbContainer_Open_Call {
	_c.Call.Return(run)
	return _c
}
